...
function buildData(){
    ...
    KE.forEach( function(ke,i_ke){
        ke_d = {};
        ke_d.R = ke.R;
        ke_d.locName = '<div class="long-text">'+ke.locName+'</div>';
        ke_d.iconToClick = '<span class="iconToClick" onclick="iconToClick('+i_ke+')" ><i class="fas fa-map-marker-alt"></i></span>';
        ke_d.eBird = '<a href="https://ebird.org/hotspot/' + ke.locId + '" target="_blank"><img src="https://zoziologie.raphaelnussbaumer.com/assets/Merge2Hotspot/images/faviconeBird16.png"></a>';
        ke_d.lat  = ke.lat;
        ke_d.lng  = ke.lng;
        ke_d.sel = "<input type='checkbox' class='selectorLocId' value='" + ke.locId + "'" + (ke.sel ? ' checked="checked" ' : '') + ">";
...
function DisplayMap(speciesCode,spDisplayName) {
    ...
    var KE_dd = KE_d.filter( ke => ke[speciesCode] !== undefined);
    var max_d = KE_dd.reduce( (acc,ke) => ke[speciesCode]>acc ? ke[speciesCode] : acc,0);
    jQuery('#canvas-max').html(max_d)

    KE_d.forEach( function(ke){
        if (ke[speciesCode] !== undefined){
            L.circleMarker([ke.lat, ke.lng], {
                radius: 8,
                color: color(ke[speciesCode],max_d),
            }).bindPopup(ke.locName_0 +'--'+ spDisplayName + '<br>Probability (%) / Number: '+ke[speciesCode]).addTo(speciesLayer)
        }
    })
}
...
KE.forEach(ke => {
    ...
    var numChecklists = ke.values_N.reduce( (acc,val) => acc += val, 0);

    var prob=[]
    Object.keys(ke.values).forEach( key=>{
        prob.push(ke.values[key].reduce( (acc,val) => acc += val ,0)/numChecklists);
    });
    
    ke.R = Math.round(prob.reduce( (acc,val) => acc += val ,0));
});
